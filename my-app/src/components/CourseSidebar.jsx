    ...
    <aside className={`fade-in-up flex min-h-[320px] flex-col rounded-3xl border border-[#FFE3B3]/45 bg-[#26648E]/30 p-5 shadow-xl backdrop-blur-xl lg:col-span-3 lg:min-h-0 ${panelClassName}`}>
      ...
      <div className="space-y-2 lg:min-h-0 lg:flex-1 lg:overflow-y-auto lg:pr-1">
          ...
          return (
            <div
              ...
              className={`space-y-2 rounded-2xl transition-all duration-300 ${
                isSelected ? "my-3 border border-[#FFE3B3]/35 bg-[#FFE3B3]/10 p-2.5" : "my-1"
              }`}
            >
              <button
                type="button"
                ...
                className={`group flex w-full cursor-pointer items-center justify-between rounded-xl border px-3 py-3 text-left text-sm transition-all duration-300 ${
                  isSelected
                    ? "border-[#FFE3B3]/85 bg-[#FFE3B3]/35 text-white shadow-md shadow-[#26648E]/40"
              ...
              {isSelected && (
                <div className="mt-1 rounded-xl border border-[#FFE3B3]/35 bg-[#FFE3B3]/12 p-2.5">
                  <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-[#FFE3B3]">
                    Learning tools for {course.code}
                  </p>
        ...
        {!courses.some((course) => course.id === selectedCourseId) && (
          <div className="mt-4 rounded-lg border border-[#FFE3B3]/30 bg-[#26648E]/35 px-3 py-2 text-xs text-[#E8F7FB]">
            Select a course to view its learning tools.
          </div>
        )}
      </div>
    </aside>
